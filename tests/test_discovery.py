...
from custom_components.tuya_selfhost.helpers import discovery
from custom_components.tuya_selfhost.helpers.discovery import (
    TuyaLANRediscovery,
...
async def test_sweep_updates_unreachable_changed_host(hass, caplog, mocker):
    ...
    mocker.patch(
        "custom_components.tuya_selfhost.helpers.discovery._find_device",
        return_value={"ip": "192.168.1.55", "id": DEVID},
    )

    with caplog.at_level(
        logging.WARNING, logger="custom_components.tuya_selfhost.helpers.discovery"
    ):
...
async def test_start_is_idempotent_and_stop_cancels(hass, mocker):
    ...
    track = mocker.patch(
        "custom_components.tuya_selfhost.helpers.discovery.async_track_time_interval",
...
async def test_product_scan_warns_once_on_unmatched_product(hass, caplog, mocker):
    ...
    mocker.patch(
        "custom_components.tuya_selfhost.helpers.discovery._scan_all",
        return_value=_scan_result(),
    )
    mocker.patch(
        "custom_components.tuya_selfhost.helpers.discovery.get_config",
